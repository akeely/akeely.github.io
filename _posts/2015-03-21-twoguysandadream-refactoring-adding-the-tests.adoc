...
To get started, I created a VM using link:https://www.vagrantup.com/[Vagrant]. I started with a link:https://www.vagrantup.com/[simple Ubuntu VM] with private networking set up so that I can connect to it from my development environment. I chose link:http://www.ansible.com/home[Ansible] to provision the VM because of it's simplicity to get started.

My link:http://www.ansible.com/home[first provisioning script] added a MySQL database to the VM. The script is configured to install MySQL, create the database, and set up permissions so that the application can connect to it.
